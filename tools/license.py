...
extra_conda_blacklist = [
    "conda",
    "conda-package-handling",
    "pycosat",
    "ruamel_yaml",
]  # need to install conda to make this script work, so we need to exclude conda from end package
    ...
    for pkg_installed in pkgs_installed:
        ...
        try:
            ...
            for build_scanned in builds_scanned:
                ...
                if (
                    build_scanned["build"] == pkg_installed["build_string"]
                    and build_scanned["build_number"] == pkg_installed["build_number"]
                ):
                    del build_scanned["channel"]
                    pkgs_scanned[pkg_installed["name"]] = {
                        **pkg_installed,
                        **build_scanned,
                    }
                    break
            ...
            print(
                f"[{Fore.YELLOW}WARN{Fore.RESET}] Package [{pkg_installed['name']}] not found in conda, maybe a pip package"
            )
...
current_license_report = read_license_csv(
    os.path.join(script_dir, "..", platform, "license_report.csv"), "name"
)
license_extra_info = read_license_csv(
    os.path.join(script_dir, "license_extra.csv"), "name"
)
...
def update_license_extra(pkgs_scanned):
    ...
    if license_extra_updated:
        ...
        write_license_csv(
            os.path.join(script_dir, "license_extra.csv"), license_extra_info
        )
...
def generate_license_report(pkgs_scanned):
    ...
    for name, pkg in get_packages(pkgs_scanned):
        ...
        if license is None:
            print(
                f"[{Fore.YELLOW}WARN{Fore.RESET}] {name} has no license in conda package metadata"
            )
            license = ""
        pkg_report[name] = pkg
    ...
    write_license_csv(
        os.path.join(script_dir, "..", os.environ["PLATFORM"], "license_report.csv"),
        pkg_report,
        fieldnames,
    )
...
def generate_app_license_file(pkgs_scanned):
    ...
    with open(os.path.join(script_dir, "..", "LICENSE")) as f:
        ...
        for name, pkg in get_packages(pkgs_scanned):
            ...
            license_packages += (
                f"{name.ljust(30)}\t{license.ljust(36)}\t{license_url}\n"
            )
